...
class TaskbarApp:
    def __init__(self):
        ...
        self.session_data = []
        self.monitoring = False  # Флаг для отслеживания состояния мониторинга
    ...
    def show_control_panel(self):
        if not self.control_panel or not self.control_panel.winfo_exists():
            ...
            Label(self.control_panel, text="Current Application:", font=('Arial', 10, 'bold')).pack(pady=5)
            self.current_app_label = Label(self.control_panel, text="", font=('Arial', 9))
            self.current_app_label.pack()
            
            Label(self.control_panel, text="Total Time:", font=('Arial', 10, 'bold')).pack(pady=5)
            self.total_time_label = Label(self.control_panel, text="", font=('Arial', 9))
            self.total_time_label.pack()
            
            Label(self.control_panel, text="Top Applications:", font=('Arial', 10, 'bold')).pack(pady=5)
            self.top_apps_text = Text(self.control_panel, height=10, width=30)
            self.top_apps_text.pack(padx=10)
            
            Button(self.control_panel, text="Start Monitoring", command=self.start_monitoring).pack(pady=5)
            Button(self.control_panel, text="Stop Monitoring", command=self.stop_monitoring).pack(pady=5)
            Button(self.control_panel, text="Download Report", command=self.download_report).pack(pady=5)
            Button(self.control_panel, text="Refresh", command=self.update_control_panel).pack(pady=5)
            Button(self.control_panel, text="Exit", command=self.exit_app).pack(pady=5)
    ...
    def update_control_panel(self):
        ...
        try:
            ...
            self.top_apps_text.insert(END, "No data available")

    def download_report(self):
        report_path = os.path.join(os.path.expanduser("~"), "Downloads", "app_report.csv")
        with open(report_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Application", "Total Time"])
    ...
    def show_context_menu(self):
        ...
        win32gui.AppendMenu(menu, win32con.MF_STRING, 1000, "Show Control Panel")
        win32gui.AppendMenu(menu, win32con.MF_STRING, 1001, "Exit")
